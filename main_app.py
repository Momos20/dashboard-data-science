...
st.caption("Funciona con cualquier CSV. 3 bloques: Cuantitativo, Cualitativo, Gráfico dinámico + Asistente.")
...
# ============================================================
# UTIL: trendline sin statsmodels (numpy polyfit)
# ============================================================
def add_linear_trendline(fig: go.Figure, data: pd.DataFrame, xcol: str, ycol: str, name="trend (lin)"):
    tmp = data[[xcol, ycol]].dropna()
    if len(tmp) < 2:
        return fig
    # proteger contra columnas no numéricas
    if not (pd.api.types.is_numeric_dtype(tmp[xcol]) and pd.api.types.is_numeric_dtype(tmp[ycol])):
        return fig

    x = tmp[xcol].to_numpy(dtype=float)
...
# ============================================================
# NODO 1 - LOADER ROBUSTO (incluye caso 1-columna)
# ============================================================
...
def load_csv_smart(file, sep, encoding, decimal, na_values, auto_fix=True):
    # intento 1: parámetros del usuario
    try:
    ...
    # intento 2: combinaciones típicas
...
# ============================================================
# PESTAÑAS (incluye Asistente)
# ============================================================
tab_qt, tab_ql, tab_g, tab_ai = st.tabs(
    ["📐 Cuantitativo", "🗂️ Cualitativo", "🎛️ Gráfico (dinámico)", "🤖 Asistente"]
)

# ============================================================
# TAB 1 - CUANTITATIVO + CORRELACIÓN
# ============================================================
with tab_qt:
    ...
    if not num_cols:
        st.error(
            ...
            "Abra Debug para ver dtypes (posible CSV en 1 sola columna o números como texto). "
            "Pruebe 'Limpiar caché y recargar'."
        )
    else:
        ...
        if len(num_cols) < 2:
            ...
            rho = corr.loc[x, y]
            st.markdown(
                f"<div class='card'><b>{method}({x}, {y})</b> = <b>{rho:.4f}</b></div>",
                unsafe_allow_html=True
            )

            fig2 = px.scatter(df_plot, x=x, y=y, opacity=0.65, title=f"{y} vs {x} (corr={rho:.4f})")
            fig2 = add_linear_trendline(fig2, df_plot, x, y, name="trend (lin)")
...
with tab_ql:
    ...
    if miss.empty:
        ...
        with c2:
            st.plotly_chart(
                px.bar(miss_df, x="columna", y="%", text="%", title="Porcentaje de nulos por columna"),
    ...
    if cat_cols:
        ...
        st.info("No se detectaron columnas categóricas (object/category).")

# ============================================================
# TAB 3 - GRÁFICO DINÁMICO
# ============================================================
with tab_g:
    ...
    if chart_type == "Histograma (numérica)":
        ...
        if len(num_cols) < 2:
            ...
            show_trend = st.checkbox("Agregar trendline (numpy)", value=True, key="g_scatter_trend")

            fig = px.scatter(df_plot, x=x, y=y, opacity=0.65, title=f"{y} vs {x}")
            if show_trend:
                fig = add_linear_trendline(fig, df_plot, x, y, name="trend (lin)")
            st.plotly_chart(fig, use_container_width=True)
        ...
        if not dt_cols:
            ...
            st.plotly_chart(px.line(tmp, x=dt, y=y, title=f"Serie temporal: {y}"), use_container_width=True)
...
# ============================================================
# TAB 4 - ASISTENTE (GROQ)
# ============================================================
with tab_ai:
    st.subheader("🤖 Asistente de análisis (Groq)")
    st.caption("Genera un análisis descriptivo con hallazgos y datos importantes (bajo demanda).")

    st.info("Requiere agregar `groq` a su requirements.txt. La API Key se ingresa en la interfaz y no se guarda.")
    ...
    def build_eda_profile_for_llm(df: pd.DataFrame, num_cols, cat_cols, dt_cols, max_rows=1000) -> dict:
        ...
        if dt_cols:
            ...
            for c in dt_cols[:10]:
                s = df[c].dropna()
                if not s.empty:
                    dt_summary[str(c)] = {"min": str(s.min()), "max": str(s.max()), "non_null": int(s.shape[0])}
    ...
    def groq_chat_completion(api_key: str, model: str, system: str, user: str) -> str:
        # import local para no romper si falta el paquete en dev
    ...
    with cB:
        st.caption("Salida: resumen + descriptivo + hallazgos + riesgos + recomendaciones (basado en perfil).")
    ...
    if run_btn:
        if not groq_api_key or len(groq_api_key.strip()) < 10:
            ...
            with st.spinner("Generando análisis con Groq..."):
                ...
                system_prompt = (
                    "Eres un analista senior de datos. Redacta un EDA profesional y accionable "
                    "basado EXCLUSIVAMENTE en el perfil suministrado. No inventes valores. "
                    "Entrega encabezados claros y bullets."
                )

                user_prompt = f"""
Con base en este perfil del dataset (dict/JSON), genere:

1) Resumen ejecutivo (3-6 bullets).
2) Análisis descriptivo: numéricas (rangos, dispersión, sesgos) y categóricas (concentración/top).
3) Hallazgos: variables dominantes, correlaciones fuertes (si existen), outliers/ceros, calidad de datos.
4) Riesgos: nulos, duplicados, tipos sospechosos, variables casi constantes, posibles errores de captura.
5) Recomendaciones: limpieza, transformaciones (log/escala), variables a revisar para modelado.
6) Si hay datetime: sugerencias de análisis temporal.

Enfoque adicional del usuario: {extra_focus if extra_focus.strip() else "N/A"}
...
"""
    ...
    if st.session_state["groq_analysis_text"]:
        ...
        st.info("Ingrese la API Key y presione **Generar análisis descriptivo** para obtener hallazgos.")
