...
st.caption("Funciona con cualquier CSV. 3 bloques: Cuantitativo, Cualitativo, Gráfico dinámico.")
...
def load_csv_smart(file, sep, encoding, decimal, na_values, auto_fix=True):
    ...
    # Si hay 1 columna y dentro del texto hay ';' o ',' repetidos, reintentar con sep adecuado.
...
# ============================================================
# ASISTENTE DE ANÁLISIS (GROQ) - PEGAR AL FINAL DEL ARCHIVO
# ============================================================
st.markdown("---")
st.subheader("🤖 Asistente de análisis (Groq)")

with st.expander("Configurar y ejecutar (API Key + análisis)", expanded=False):
    st.caption("El análisis se ejecuta bajo demanda. La API Key no se guarda en el repositorio.")
...
def build_eda_profile_for_llm(df: pd.DataFrame, num_cols, cat_cols, dt_cols, max_rows=1000) -> dict:
    ...
    # Optional: sample rows (small) for context (no PII), limited
    sample = df.head(max_rows).copy()
    # Convertir a strings para que sea JSON-able
...
with colA:
    run_btn = st.button("🧠 Generar análisis descriptivo", type="primary", use_container_width=True)

with colB:
    st.caption("El asistente genera: resumen descriptivo + hallazgos + riesgos de calidad + variables clave + recomendaciones.")
